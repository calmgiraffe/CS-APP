...
int isAsciiDigit(int x) {
	int mask = (0xFF << 24) + (0xFF << 16) + (0xFF << 8) + 0xF0;
	return !(((x | 0x30) & mask) ^ 0x30) & !(((x + 0x06 | 0x30) & mask) ^ 0x30) & !(x & 0x30 ^ 0x30);
}
/* 
...
unsigned floatScale2(unsigned uf) {
	// if exp == 00000000, shift bits right by 1 and preserve sign
	// if exp == 11111111, return arg
	...
	if (!(uf & expMask)) {
		...
		return (uf << 1) + sign;
	} else if ((uf & expMask) + expIncrement & signMask) {
		return uf;
	} else {
		int newexp = (uf & expMask) + expIncrement;
		return newexp + (uf & 0x807fffff);
	}
}
/* 
